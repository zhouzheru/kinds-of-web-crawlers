...
def douban(number):
    ...
    for num in range(0,45,15):
        base_url = 'https://movie.douban.com/people/138048749/wish?start='+str(num)+'&' #这里的base_url 需要自己更改
        params = {'sort':'time',
                "rating":'all',
                'filter':'all',
                'mode':'grid'}
    ...
    print('以下是你看过的电影：')
    for num in range(0,45,15):
        base_url = 'https://movie.douban.com/people/138048749/collect?start='+str(num)+'&' ##这里的base_url 需要自己更改
        params = {'sort':'time',
                "rating":'all',
                'filter':'all',
                'mode':'grid'}
